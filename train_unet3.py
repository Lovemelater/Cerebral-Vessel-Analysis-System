...
def train():
    ...
    # 使用axis=2（矢状面切片）- 这里可以根据需要调整
    train_dataset = MRADataset(train_data, train_label, axis=2)
    val_dataset = MRADataset(val_data, val_label, axis=2)
    ...
    for epoch in range(50):
        ...
        print(f'Epoch {epoch+1}/50 - Train Loss: {avg_train_loss:.4f}, Train Dice: {avg_train_dice:.4f} - Val Loss: {avg_val_loss:.4f}, Val Dice: {avg_val_dice:.4f} - LR: {current_lr:.6f}')
        ...
        if avg_train_dice > best_train_dice:
            ...
            torch.save(model.state_dict(), 'unet3_best_t.pth')
        ...
        if avg_val_dice > best_val_dice:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), 'unet3_best_v.pth')
