...
def predict3(file_name="D:/brain_data/Normal-001/MRA/Normal001-MRA.mha", i=3):
    try:
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        print(f"使用设备: {device}")
        
        # 加载模型
        model_path = 'unet3_best_t.pth'  # 根据你实际保存的模型文件名调整
        ...
        slices = data.shape[2]  # axis=2 对应矢状面切片
        
        # 逐切片预测（沿axis=2方向）
        results = []
        for slice_idx in range(slices):
            data_slice = data[:, :, slice_idx]  # 提取矢状面切片
        ...
        result_volume = np.stack(results, axis=2)  # axis=2对应训练时的切片方向
        ...
        save_path = os.path.join('mha', f'test3_{i}.mha')
