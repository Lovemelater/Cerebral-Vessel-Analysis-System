...
def predict1(file_name="D:/brain_data/Normal-001/MRA/Normal001-MRA.mha", i=1):
    try:
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
        print(f"使用设备: {device}")
        
        # 加载模型
        model_path = 'unet1_best_t.pth'  # 根据你实际保存的模型文件名调整
        ...
        # 合并结果
        result_volume = np.stack(results, axis=0)
        print(f"预测结果范围: {result_volume.min()}, {result_volume.max()}")
        ...
        save_path = os.path.join('mha', f'test1_{i}.mha')
